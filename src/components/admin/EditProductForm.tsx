...
import { editProducts } from "@/lib/api/action/action"; // ⚠️ adjust to your real export path

// 💡 1. Updated FoodItem Type Definition (_id mandatory to fix TS build error)
export interface FoodItem {
  _id: string; // 👈 _id required string
  name?: string;
  description?: string;
  image?: string;
  price?: number | string;
...
// 💡 2. Flexible Props Interface
export interface EditProductModalProps {
  product: FoodItem;
  productId?: string;
  onClose?: () => void;
  onUpdate?: (updatedProduct: FoodItem) => void;
}
...
export default function EditProductModal({
  ...
  useEffect(() => {
    if (
      product?.measurement_value?.toLowerCase().includes("gm") ||
      product?.measurement_value?.toLowerCase().includes("kg")
    ) {
      setUnitType("weight");
    } else {
  ...
  // 💡 3. Smart handleClose: uses onClose if modal, router.back() if page
  const handleClose = () => {
    if (onClose) {
      onClose();
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Ensure _id is maintained properly in merged product
      const mergedProduct: FoodItem = {
        ...product,
        ...updatedData,
        _id: targetId,
      };

      await editProducts(targetId, mergedProduct);
      ...
      if (onClose) {
        ...
        router.push("/foods"); // ⚠️ adjust to your real route
      }
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-3xl max-h-[92vh] bg-white rounded-3xl shadow-2xl shadow-amber-950/30 overflow-hidden flex flex-col ring-1 ring-amber-100">
        ...
        >
          ...
          {/* Section: Measurement */}
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-amber-600/80 mb-3">
              Measurement
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gradient-to-br from-amber-50 to-rose-50/50 border border-amber-200/80 rounded-2xl">
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    value={unitType}
                    onChange={(e) =>
                      setUnitType(e.target.value as "weight" | "quantity")
                    }
              ...
              <div>
                ...
                <input
                  ...
                  defaultValue={product?.measurement_value}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    type="date"
                    name="creatingDate"
                    defaultValue={toDateInputValue(product?.creatingDate)}
                    className="w-full pl-11 pr-4 py-2.5 bg-amber-50/50 border border-amber-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent text-amber-900 transition-shadow"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    type="date"
                    name="expireDate"
                    defaultValue={toDateInputValue(product?.expireDate)}
                    className="w-full pl-11 pr-4 py-2.5 bg-amber-50/50 border border-amber-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent text-amber-900 transition-shadow"
                  />
                </div>
              </div>
